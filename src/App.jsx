...
function AstralScreen({profile, goHumor, goTest, goQuizzes}) {
  ...
  return (
    <div>
      ...
      <Card onClick={()=>goTest&&goTest("deusa")} style={{cursor:"pointer",marginBottom:14,borderLeft:"3px solid #10B981"}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <div style={{flex:1}}>
            ...
            {profile?.deusa && DEUSA_INFO[profile.deusa] && <div style={{fontSize:10,color:"#10B981",fontWeight:700,marginTop:4}}>✅ {DEUSA_INFO[profile.deusa].emoji} {DEUSA_INFO[profile.deusa].nome}</div>}
          </div>
          <span style={{fontSize:16,color:"#10B981",opacity:.6}}>→</span>
        </div>
      </Card>
  ...
  return (
    <div>
      ...
      {[
        ...
        if (t.pro) {
          return (
            <Card key={t.key} style={{cursor:"default",borderLeft:"4px solid #9CA3AF",marginBottom:10,opacity:0.65,position:"relative",overflow:"hidden"}}>
...
function TestesScreen({profile, onComplete, goCoincidence, onSaveExtra, initialAba, autoStart, goAstral, goEspiritualidadeTeste, soTestes, soQuizzes}) {
  ...
  const startNovo = (testKey) => {
    if (PRO_TESTES.includes(testKey)) return; // bloqueado — PRO
  ...
  if (aba==="menu") {
    ...
    const novosCards = [
      ...
      {key:"profissional", emoji:"💼", nome:"Perfil Profissional", desc:"15 perguntas · seu estilo no trabalho", cor:"#10B981", pro:true},
      {key:"politico", emoji:"🗳️", nome:"Espectro Político", desc:"15 perguntas · Esquerda, Centro, Direita ou Liberal", cor:"#3B82F6", pro:true},
      {key:"lider", emoji:"👑", nome:"Qual Líder Histórico Você Seria", desc:"12 perguntas · seu estilo de liderança", cor:"#8B5CF6"},
      {key:"financeiro", emoji:"💰", nome:"Seu Perfil Financeiro", desc:"12 perguntas · sua relação com o dinheiro", cor:"#10B981", pro:true},
    ...
    return (
      <div>
        ...
        {/* Demais testes — ORDEM FIXA (só livres) */}
        {novosCards.filter(nc=>!nc.pro).map(nc => (
        ...
        {/* ── TESTES PRO (no final) ── */}
        {novosCards.filter(nc=>nc.pro).length > 0 && (
          <>
            <div style={{fontSize:11,color:"#9CA3AF",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginTop:24,marginBottom:12}}>⭐ Em Breve · Versão PRO</div>
            {novosCards.filter(nc=>nc.pro).map(nc => (
              <Card key={nc.key} style={{cursor:"default",borderLeft:"4px solid #9CA3AF",opacity:0.65,position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",top:10,right:12,background:"#0A1F1A",color:"#10B981",fontSize:9,fontWeight:800,padding:"3px 9px",borderRadius:20,letterSpacing:1}}>🔒 PRO</div>
                <div style={{display:"flex",gap:14,alignItems:"center"}}>
                  <div style={{width:52,height:52,borderRadius:16,background:"#F3F4F6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,filter:"grayscale(1)"}}>{nc.emoji}</div>
                  <div style={{flex:1,paddingRight:50}}>
                    <div style={{fontSize:15,fontWeight:800,color:"#9CA3AF",marginBottom:2}}>{nc.nome}</div>
                    <div style={{fontSize:12,color:"#9CA3AF"}}>{nc.desc}</div>
                  </div>
                </div>
              </Card>
        ...
        {Object.entries(QUIZ_BANKS).filter(([key])=>!["curiosidades","elementos","proverbios","qi"].includes(key)).map(([key,qb])=>{
        ...
        {Object.entries(QUIZ_BANKS).filter(([key])=>["curiosidades","elementos","proverbios","qi"].includes(key)).map(([key,qb])=>{
          const totalQ = qb.niveis.facil.length + qb.niveis.medio.length + qb.niveis.dificil.length;
          return (
            <Card key={key} style={{cursor:"default",borderLeft:"4px solid #9CA3AF",opacity:0.65,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:10,right:12,background:"#0A1F1A",color:"#10B981",fontSize:9,fontWeight:800,padding:"3px 9px",borderRadius:20,letterSpacing:1}}>🔒 PRO</div>
              <div style={{display:"flex",gap:14,alignItems:"center"}}>
                <div style={{width:52,height:52,borderRadius:16,background:"#F3F4F6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,filter:"grayscale(1)"}}>{qb.emoji}</div>
                <div style={{flex:1,paddingRight:50}}>
                  <div style={{fontSize:15,fontWeight:800,color:"#9CA3AF",marginBottom:2}}>{qb.nome}</div>
                  <div style={{fontSize:12,color:"#9CA3AF"}}>3 níveis · {totalQ} perguntas</div>
                </div>
              </div>
            </Card>
