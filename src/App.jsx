...
const renderRaioX = (ctx, { nome, resultados }) => {
  ...
  resultados.slice(0, 6).forEach((r) => {
    ...
    ctx.fillText(r.valor, 260, yPos + 100);
...
const renderMundo = (ctx, { dataStr, calendarios }) => {
  ...
  ctx.fillText("Em que ano estamos?", W/2, 490);
...
const renderHoroscopo = (ctx, { nome, signo, emoji, sistema, mensagem }) => {
  ...
  if (nome) {
    ...
    ctx.font = "bold 36px 'Sora', sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(nome.toUpperCase(), W/2, 730);
  }
  ...
  ctx.shadowBlur = 20;
  ctx.fillStyle = BRAND.white;
  ctx.font = "900 120px 'Sora', sans-serif";
  ctx.textAlign = "center";
  ...
  ctx.fillText(`Astrologia ${sistema}`, W/2, 905);
...
const renderNumerologia = (ctx, { nome, numero, arquetipo, essencia }) => {
  ...
  if (nome) {
    ...
    ctx.textAlign = "center";
    ctx.fillText(nome.toUpperCase(), W/2, 410);
  }
...
// ============================================================
function CoincidenceTest({profile, onSaveToProfile, onBack, goPerfil}) {
  ...
  if (step==="input") return (
    <div>
      ...
      {/* Card MEU PERFIL — destacado */}
      {isMainTab && (
        <div onClick={goPerfil} style={{cursor:"pointer",background:"linear-gradient(135deg,#10B981,#059669)",borderRadius:18,padding:"14px 16px",marginBottom:16,boxShadow:"0 4px 14px rgba(16,185,129,0.25)",display:"flex",alignItems:"center",gap:14}}>
          <div style={{width:52,height:52,borderRadius:"50%",background:"#ffffff25",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:800,color:"#fff",flexShrink:0}}>
...
// ============================================================
function TestesScreen({profile, onComplete, goCoincidence, onSaveExtra, initialAba, goAstral}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F8FAFB",fontFamily:"'Sora',sans-serif",paddingBottom:90}}>
      ...
      <div style={{maxWidth:440,margin:"0 auto",padding:"20px 16px 0",animation:"fadeUp .35s"}}>
        {tab==="komparai" && <CoincidenceTest profile={profile} onSaveToProfile={saveToProfile} onBack={()=>setTab("komparai")} goPerfil={()=>setTab("perfil")}/>}
        ...
        {tab==="testes" && <TestesScreen profile={profile} onComplete={onTestDone} goCoincidence={()=>setTab("komparai")} onSaveExtra={saveToProfile} initialAba={testesInitialAba} goAstral={testesInitialAba==="humor" ? ()=>{setTestesInitialAba(null); setTab("astral");} : null}/>}
        {tab==="coincidencia" && <CoincidenceTest profile={profile} onSaveToProfile={saveToProfile} onBack={()=>setTab("komparai")}/>}
