...
const QUIZ_BANKS = {
  ...
  bandeiras: {
    emoji: "🚩", nome: "Bandeiras do Mundo", cor: "#EF4444", desc: "Reconheça os países",
    niveis: {
      facil: [
        { q: "Qual país tem uma folha de bordo vermelha na bandeira?", opts: ["EUA","Canadá","Suíça","Áustria"], c: 1 },
        { q: "Bandeira branca com um círculo vermelho no centro é de qual país?", opts: ["China","Coreia do Sul","Japão","Vietnã"], c: 2 },
        { q: "Verde e amarela com losango e círculo azul é a bandeira do(a):", opts: ["Brasil","Jamaica","Camarões","Gana"], c: 0 },
        { q: "Bandeira com listras vermelhas e brancas e estrelas no canto azul:", opts: ["Reino Unido","EUA","Libéria","Malásia"], c: 1 },
        { q: "Azul, branco e vermelho em listras verticais (original):", opts: ["Itália","Rússia","França","Holanda"], c: 2 },
      ],
      medio: [
        { q: "Cruz vermelha sobre fundo branco (cruz de São Jorge):", opts: ["Dinamarca","Inglaterra","Suíça","Geórgia"], c: 1 },
        { q: "Bandeira com um sol no centro de listras azul e branca:", opts: ["Uruguai","Argentina","Grécia","Israel"], c: 1 },
        { q: "Vermelha com uma estrela grande e quatro menores:", opts: ["Vietnã","China","Turquia","Cuba"], c: 1 },
        { q: "Cruz amarela sobre fundo azul:", opts: ["Finlândia","Noruega","Suécia","Islândia"], c: 2 },
        { q: "Três faixas horizontais: preta, vermelha e dourada:", opts: ["Bélgica","Alemanha","Espanha","Angola"], c: 1 },
      ],
      dificil: [
        { q: "Qual bandeira tem um dragão?", opts: ["Butão","Nepal","Sri Lanka","Mongólia"], c: 0 },
        { q: "Bandeira com um cedro verde no centro:", opts: ["Líbano","Chipre","Canadá","Eritreia"], c: 0 },
        { q: "Qual é a única bandeira nacional que não é retangular?", opts: ["Suíça","Nepal","Vaticano","Catar"], c: 1 },
        { q: "Qual bandeira tem um fuzil AK-47?", opts: ["Angola","Moçambique","Zimbábue","Congo"], c: 1 },
        { q: "Bandeira com uma águia comendo uma serpente:", opts: ["Egito","México","Albânia","Equador"], c: 1 },
      ],
    },
  },
  capitais: {
...
function CoincidenceTest({profile, onSaveToProfile, onBack, goPerfil, goTest}) {
  ...
  return (
    <div>
      ...
      {/* ── #2 Mapa de coincidências ── */}
      {results.length > 0 && total > 0 && (() => {
        const signEmojis = new Set(Object.values(SIGN_EMOJI));
        const cats = [
          { icon:"🎂", label:"Aniversário", cels: results.filter(r=>r.hits.some(h=>h.icon==="🎂")) },
          { icon:"🗓️", label:"Mesmo mês", cels: results.filter(r=>r.hits.some(h=>h.icon==="🗓️")) },
          { icon:"🎉", label:"Mesma geração", cels: results.filter(r=>r.hits.some(h=>h.icon==="🎉")) },
          { icon:"✨", label:"Mesmo signo", cels: results.filter(r=>r.hits.some(h=>signEmojis.has(h.icon))) },
          { icon:"📏", label:"Altura parecida", cels: results.filter(r=>r.hits.some(h=>h.icon==="📏")) },
          { icon:"⚖️", label:"Peso parecido", cels: results.filter(r=>r.hits.some(h=>h.icon==="⚖️")) },
          { icon:"🧬", label:"Temperamento", cels: results.filter(r=>r.hits.some(h=>h.icon==="🧬")) },
        ].filter(c=>c.cels.length>0);
        if (!cats.length) return null;
        return (
          <div style={{background:"#F0FDF4",borderRadius:18,padding:"14px 16px",marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:800,color:"#0A1F1A",marginBottom:10}}>🗺️ Mapa de coincidências</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {cats.map(cat=>(
                <div key={cat.label} style={{background:"#fff",borderRadius:12,padding:"9px 12px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:cat.cels.length<=4?4:2}}>
...
function TestesScreen({profile, onComplete, goCoincidence, onSaveExtra, initialAba, autoStart, goAstral, goEspiritualidadeTeste}) {
  ...
  const answerQuiz = (idx) => {
    ...
    setTimeout(()=>{ setQFb(null); setQPicked(null); if (qIdx<qPool.length-1) setQIdx(i=>i+1); else { setQDone(true); playSfx("win"); } }, 950);
  };
