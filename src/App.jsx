...
function TestesScreen({profile, onComplete, goCoincidence, onSaveExtra, initialAba, autoStart, goAstral, goEspiritualidadeTeste}) {
  ...
  const openGenQuiz = (key) => { setGqKey(key); setGqLevel(null); setGqPool([]); setGqIdx(0); setGqScore(0); setGqDone(false); setGqFb(null); setAba("genquiz"); };
  const startGenLevel = (level) => {
    const pool = shuffle(QUIZ_BANKS[gqKey].niveis[level]);
    setGqLevel(level); setGqPool(pool); setGqIdx(0); setGqScore(0); setGqDone(false); setGqFb(null);
  };
  ...
  const resetGenQuiz = () => { setAba("menu"); setGqKey(null); setGqLevel(null); setGqPool([]); setGqIdx(0); setGqScore(0); setGqDone(false); setGqFb(null); setGqPicked(null); };
    ...
    return (
      <div>
        ...
        {Object.entries(QUIZ_BANKS).map(([key,qb])=>{
          ...
          const feitos = QUIZ_NIVEIS.filter(n => (profile?.[`qb_${key}_${n.id}`]!=null));
          return (
                ...
                <div style={{flex:1}}>
                  ...
                  {feitos.length>0 && (
                    <div style={{display:"flex",gap:5,marginTop:5,flexWrap:"wrap"}}>
                      {feitos.map(n=>(
                        <span key={n.id} style={{fontSize:9,fontWeight:700,color:n.cor,background:`${n.cor}15`,borderRadius:20,padding:"2px 7px"}}>
                          {n.emoji} {profile[`qb_${key}_${n.id}`]}/{qb.niveis[n.id].length}
                        </span>
                      ))}
                    </div>
                  )}
  ...
  if (aba==="genquiz" && gqKey) {
    const qb = QUIZ_BANKS[gqKey];
    // Seleção de nível
    if (!gqLevel) {
      return (
        <div>
          <TestHeader emoji={qb.emoji} nome={qb.nome} cor={qb.cor} sub={qb.desc} onCancel={resetGenQuiz}/>
          <div style={{fontSize:13,color:"#9CA3AF",marginBottom:14,fontWeight:600}}>Escolha o nível</div>
          {QUIZ_NIVEIS.map(lv=>{
    ...
    const nivel = QUIZ_NIVEIS.find(n=>n.id===gqLevel);
    // Resultado
    if (gqDone) {
      const total = gqPool.length, pct = Math.round((gqScore/total)*100);
      const best = profile?.[`qb_${gqKey}_${gqLevel}`] || 0;
      const novoRecorde = gqScore >= best && gqScore > 0;
      ...
      return (
        <div>
          ...
          <Btn label="↻ Jogar de novo" onClick={()=>startGenLevel(gqLevel)}/>
          <div style={{marginTop:10}}><Btn label="📊 Trocar de nível" onClick={()=>{setGqLevel(null);setGqDone(false);}} ghost/></div>
    ...
    return (
      <div>
        <TestHeader emoji={qb.emoji} nome={qb.nome} cor={qb.cor} sub={`Nível ${nivel.nome}`} step={gqIdx+1} total={gqPool.length} score={gqScore} onCancel={resetGenQuiz}/>
